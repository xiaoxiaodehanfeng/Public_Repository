    ...
    def test_getInvestsByMemberId(self, case):
       ...
       # 判断返回里面是否有cookies(有说明是登录接口)
        ...
        if resp_dict['msg'] == "获取用户投资列表成功":
            ...
            sql_select_invest = "SELECT * FROM future.invest WHERE MemberID = {} ORDER BY Id DESC;".format(Context.invest_member_id)
            invest_list = self.mysql.fetch_all(sql_select_invest)
            # 根据用户ID获取invest表中某个用户的投资记录的最大行数
            sql_select_row = "SELECT COUNT(Id) FROM future.invest WHERE MemberID = {};".format(Context.invest_member_id)
